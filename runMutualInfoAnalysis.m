...
% Spike Train Params
snippetLength = [10, 20, 50, 100]; % ms
spikeTrainRes = 1;
stParams.snippetLength = snippetLength; stParams.spikeTrainRes = spikeTrainRes;

% ISI Params
isiRes = 1; % ms
isiParams.isiTimeWindow = [1, 2, 5, 10, 20, 25, 50, 100]; % ms
isiTau = 1:15; % 1:19
...
% Processing Loop
for ss = 1:length(isiRes)
    
    % Get ISI Data
    isiHist = getISIHist(cData, startTime, poiDur, isiRes(ss), isiCutoff);
    isiEdges = 0:isiRes(ss):isiCutoff;
    spikeTrain = binSpikeData(cData, spikeTrainRes(ss), startTime, poiDur);
    
    for ll = 1:length(isiParams.isiTimeWindow)
        clc; fprintf('ISI Res: %i/%i, ISI Length %i/%i\n',  length(isiRes), ll, length(isiParams.isiTimeWindow));

        isiLimit = find(isiEdges <= isiParams.isiTimeWindow(ll), 1, 'last');
